...
const Analysis = () => {
    ...
    let [showAdvancedOptions_LinearRegression, setShowAdvancedOptions_LinearRegression] = useState(false)
    let [metrics_LinearRegression,setMetrics_LinearRegression] = useState('Select metrics')
    let [showOptions_LinearRegression, setShowOptions_LinearRegression] = useState(false)


    let [metrics_DecisionTreeRegression,setMetrics_DecisionTreeRegression] = useState('Select metrics')
    let [showOptions_DecisionTreeRegression, setShowOptions_DecisionTreeRegression] = useState(false)
    let [showAdvancedOptions_DecisionTreeRegression, setShowAdvancedOptions_DecisionTreeRegression] = useState(false)

    let [metrics_RandomForestRegression,setMetrics_RandomForestRegression] = useState('Select metrics')
    let [showOptions_RandomForestRegression, setShowOptions_RandomForestRegression] = useState(false)
    let [showAdvancedOptions_RandomForestRegression, setShowAdvancedOptions_RandomForestRegression] = useState(false)

    let [metrics_SVMRegression,setMetrics_SVMRegression] = useState('Select metrics')
    let [showOptions_SVMRegression, setShowOptions_SVMRegression] = useState(false)
    let [showAdvancedOptions_SVMRegression, setShowAdvancedOptions_SVMRegression] = useState(false)

    let [metrics_LogisiticRegression,setMetrics_LogisiticRegression] = useState('Select metrics')
    let [showOptions_LogisiticRegression, setShowOptions_LogisiticRegression] = useState(false)
    let [showAdvancedOptions_LogisiticRegression, setShowAdvancedOptions_LogisiticRegression] = useState(false)

    let [metrics_DecisionTreeClassifier,setMetrics_DecisionTreeClassifier] = useState('Select metrics')
    let [showOptions_DecisionTreeClassifier, setShowOptions_DecisionTreeClassifier] = useState(false)
    let [showAdvancedOptions_DecisionTreeClassifier, setShowAdvancedOptions_DecisionTreeClassifier] = useState(false)

    let [metrics_RandomForestClassifier,setMetrics_RandomForestClassifier] = useState('Select metrics')
    let [showOptions_RandomForestClassifier, setShowOptions_RandomForestClassifier] = useState(false)
    let [showAdvancedOptions_RandomForestClassifier, setShowAdvancedOptions_RandomForestClassifier] = useState(false)

    let [metrics_NaiveBayesClassifier,setMetrics_NaiveBayesClassifier] = useState('Select metrics')
    let [showOptions_NaiveBayesClassifier, setShowOptions_NaiveBayesClassifier] = useState(false)
    let [showAdvancedOptions_NaiveBayesClassifier, setShowAdvancedOptions_NaiveBayesClassifier] = useState(false)
    ...
    let [metrics_agglomerative,setMetrics_agglomerative] = useState('Select metrics')
    let [showOptions_agglomerative, setShowOptions_agglomerative] = useState(false)
    let [showAdvancedOptions_agglomerative, setShowAdvancedOptions_agglomerative] = useState(false)

    let [metrics_apriori,setMetrics_apriori] = useState('Select metrics')
    let [showOptions_apriori, setShowOptions_apriori] = useState(false)
    let [showAdvancedOptions_apriori, setShowAdvancedOptions_apriori] = useState(false)
        ...
        <Modal isOpen={showSubOptionModal} onClose={async ()=>{
            ...
            <div className='p-5 flex flex-col'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                    ...
                    <Input onInput={e=>{
                        result.test_size_dtr = e.target.value
                    }} customStyle={`w-64 ${showAdvancedOptions_DecisionTreeClassifier?'':''}`} attrs={{list:'test_size_dtc_list'}} />

                    <Label customStyle={`${showAdvancedOptions_DecisionTreeClassifier?'':''}`} text='Set parameters: max_depth'/>
                    <Input onInput={e=>{
                        result.param_max_depth = e.target.value
                    }} customStyle={`w-64 ${showAdvancedOptions_DecisionTreeClassifier?'':''}`} attrs={{list:'max_depth_dtc_list'}} />


                    <Label text='Classification Result:'/>
                    <DropDown defaultText={'Select'}  customStyle={`w-64`}  customUlStyle={`w-64`} showOnHover={false} text={metrics_DecisionTreeClassifier} items={['Visualize Tree: Text Graph', 'Visualize Tree: Flowchart', 'Predicted VS. Observed'].map((item,i)=>{
                        let onClick = ()=>{
                            setMetrics_DecisionTreeClassifier(item)
                            if(i==2){
                            setShowOptions_DecisionTreeClassifier(1)
                            }else{
                                setShowOptions_DecisionTreeClassifier(0)
                            }
                        }
                        return {
                            name:item,onClick
                    ...
                    <Label customStyle={`${showOptions_DecisionTreeClassifier?'':'hidden'}`} text='Plot Types'/>
                    <DropDown defaultText={'Select type'} showOnHover={false} customStyle={`${showOptions_DecisionTreeClassifier?'':'hidden'} w-64`} customUlStyle={`w-64`} items={['Scatter', 'Bar', 'Line', 'Heatmap']} onSelect={e=>{}} />
                    ...
                    <Input onInput={e=>{
                        result.find_max_depth = e.target.value
                    }} customStyle={`w-64 ${showAdvancedOptions_DecisionTreeClassifier?'':''}`} attrs={{list:'find_max_depth_dtc_list'}} />
           

                    <Checkbox label="Show Advanced Options" defaultChecked={false} onClick={e=>{
                        setShowAdvancedOptions_DecisionTreeClassifier(e.target.checked)
                    }}/>
                    <Label text=''/>
                    <Label customStyle={`${showAdvancedOptions_DecisionTreeClassifier?'':'hidden'}`} text='Set Parameters: criterion'/>
                    <DropDown defaultText={'Select criterion'} showOnHover={false} customStyle={`w-64 ${showAdvancedOptions_DecisionTreeClassifier?'':'hidden'}`}  customUlStyle='w-64' items={['gini', 'entropy']}
                    onSelect={name=>{
                        result.param_criterion = name
                    }} />
                    <Label customStyle={`${showAdvancedOptions_DecisionTreeClassifier?'':'hidden'}`} text='Set Parameters: max_leaf_nodes'/>
                     <Input onInput={e=>{
                        result.param_max_leaf_nodes = e.target.value
                    }} customStyle={`w-64 ${showAdvancedOptions_DecisionTreeClassifier?'':''}`} attrs={{list:'max_leaf_nodes_dtc_list'}} />
                    ...
                    <datalist id="test_size_dtc_list"><option value="30"></option><option value="20"></option><option value="10"></option></datalist>
                    <datalist id="max_depth_dtc_list"><option value="5"></option><option value="6"></option><option value="7"></option></datalist>
                    <datalist id="find_max_depth_dtc_list"><option value="1,2,3,4,5,10,15,20,25,50,100"></option></datalist>
                    <datalist id="max_leaf_nodes_dtc_list"><option value="20"></option><option value="25"></option></datalist>
            ...
            <div className='mx-5 my-10 w-12/12 w-full flex justify-start'>
                <div className='w-72'>
                    <DropDown text={optionText} customStyle='h-10 w-72' customUlStyle={'w-72'} items={
                        ['Regression', 'Classification', 'Clustering', 'Associate Rule', 'Time Series Analysis'].map((item, i) => ({
                            name: item, onClick(e) {
                                {/*   0                           1                            2                               3                 4    */ }
                                setOption(i)
                                setOptionText(item)
